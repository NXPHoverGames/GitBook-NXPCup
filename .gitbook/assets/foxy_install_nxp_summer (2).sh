# NXP Gazebo dependencies install script
# Landon Haugh (landon.haugh@nxp.com)
# Benjamin Perseghetti (bperseghetti@rudislabs.com)

# Function to catch errors and redirect users to contact Landon
catch_errors () {
	if [[ $? > 0 ]]
	then
		echo "The command failed.. contact Landon Haugh (landon.haugh@nxp.com) for support"
		exit
	else
...
#
# Time to operate some robots
#

# install curl.. because apparently it doesn't exist on ubuntu by default
...
# Install Python 3 required packages
sudo apt-get install -y python3-argcomplete python3-empy python3-jinja2 \
	python3-cerberus python3-coverage python3-matplotlib python3-numpy \
       	python3-packaging python3-pkgconfig python3-opencv \
	python3-colcon-common-extensions python3-wheel python3-setuptools \
	python3-requests python3-serial python3-six python3-toml \
	python3-psutil python3-jinja2 python3-pysolar
catch_errors

# Check to see if things are already sourced... this script may fail and then be run again by the user.
if ! grep -q "source /opt/ros/foxy/setup.bash" "/home/$USER/.bashrc"; then
...
echo
echo '-------------------------------------------------------------------------------'
echo 'DONE - Please continue to the section titled "Installing NXP Gazebo". Have fun!'
echo '-------------------------------------------------------------------------------'
